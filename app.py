...
app.config['TESTING'] = _IS_TESTING
    ...
    if request.method == 'POST':
        ...
        if user_type == 'professor':
            ...
            if professor:
                ...
                if professor.failed_login_attempts >= 5:
                    professor.account_locked = True
                db.session.commit()
            app.logger.info('[audit] professor login fail: ident=%s ip=%s', ident, request.remote_addr)
...
def reset_password(token):
    ...
    if request.method == 'POST':
        ...
        err = _validate_password_strength(pwd)
        if err:
            return jsonify({'success': False, 'error': err})
        if pwd != confirm:
        ...
        professor.failed_login_attempts = 0
        professor.account_locked = False
        db.session.commit()
...
def logout():
    ...
    app.logger.info('[audit] professor logout: id=%s', current_user.id)
    logout_user()
    return redirect(url_for('login'))
...
def start_class(class_id):
    ...
    db.session.commit()
    app.logger.info('[audit] class start: professor_id=%s class_id=%s', current_user.id, class_id)
...
def stop_class(class_id):
    ...
    db.session.commit()
    app.logger.info('[audit] class stop: professor_id=%s class_id=%s', current_user.id, class_id)
...
def export_gradebook(class_id):
    ...
    app.logger.info('[audit] gradebook export: professor_id=%s class_id=%s', current_user.id, class_id)
...
def export_students(class_id):
    ...
    if class_obj.professor_id != current_user.id:
        return jsonify({'success': False, 'error': 'Unauthorized'}), 403
    app.logger.info('[audit] student export: professor_id=%s class_id=%s', current_user.id, class_id)
...
@app.route('/api/student/<int:student_id>/data', methods=['DELETE'])
@login_required
def delete_student_data(student_id):
    ...
    student.last_name = '[deleted]'
    student.preferred_name = None
    student.email = 'deleted@deleted'
    student.student_number = 'DEL' + str(student.id)
    student.rfid_card_id = None
    student.password_hash = None
    student.failed_login_attempts = 0
    student.password_locked = False
    db.session.commit()
    app.logger.info('[audit] student data deleted: professor_id=%s student_id=%s', current_user.id, student_id)
    return jsonify({'success': True})
...
def student_login():
    ...
    # RFID (hardware nameplates) — lockout does not apply to hardware auth
    if rfid_card_id:
    ...
    if identifier:
        ...
        if not student:
            app.logger.info('[audit] student login fail: ip=%s', request.remote_addr)
        ...
        if 'student' in table_names:
            ...
            for col_name, col_def in [
                ...
                if col_name not in student_columns:
                    try:
                        db.session.execute(text(f'ALTER TABLE student ADD COLUMN {col_name} {col_def}'))
                        db.session.commit()
                        print(f"[OK] Added {col_name} to student table")
                    except Exception as e:
                        db.session.rollback()
                        print(f"[ERROR] {e}")

        # Check if professor table exists and add lockout columns
        if 'professor' in table_names:
            professor_columns = [col['name'] for col in inspector.get_columns('professor')]
            for col_name, col_def in [
                ('failed_login_attempts', 'INTEGER DEFAULT 0'),
                ('account_locked', 'BOOLEAN DEFAULT 0'),
            ]:
                if col_name not in professor_columns:
                    try:
                        db.session.execute(text(f'ALTER TABLE professor ADD COLUMN {col_name} {col_def}'))
                        db.session.commit()
                        print(f"[OK] Added {col_name} to professor table")
                    except Exception as e:
                        db.session.rollback()
                        print(f"[ERROR] {e}")
